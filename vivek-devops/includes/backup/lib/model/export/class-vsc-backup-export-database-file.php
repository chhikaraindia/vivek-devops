...
class VSC_Backup_Export_Database_File {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['total_database_size'] ) ) {
			...
			$total_database_size = vsc_backup_database_bytes( $params );
		}
		...
		// Set progress
		VSC_Backup_Status::info( sprintf( __( 'Archiving database...<br />%d%% complete', VSC_BACKUP_PLUGIN_NAME ), $progress ) );
		...
		if ( $archive->add_file( vsc_backup_database_path( $params ), VSC_BACKUP_DATABASE_NAME, $database_bytes_written, $database_bytes_offset ) ) {

			// Set progress
			VSC_Backup_Status::info( __( 'Done archiving database.', VSC_BACKUP_PLUGIN_NAME ) );
