...
class VSC_Backup_Export_Config {

	public static function execute( $params ) {
		global $table_prefix, $wp_version;

		// Set progress
		VSC_Backup_Status::info( __( 'Preparing configuration file...', VSC_BACKUP_PLUGIN_NAME ) );
		...
		// Set plugin version
		$config['Plugin'] = array( 'Version' => VSC_BACKUP_VERSION );

		// Set WordPress version and content
		$config['WordPress'] = array( 'Version' => $wp_version, 'Content' => WP_CONTENT_DIR, 'Plugins' => vsc_backup_get_plugins_dir(), 'Themes' => vsc_backup_get_themes_dirs(), 'Uploads' => vsc_backup_get_uploads_dir(), 'UploadsURL' => vsc_backup_get_uploads_url() );
		...
		// Set active plugins
		$config['Plugins'] = array_values( array_diff( vsc_backup_active_plugins(), vsc_backup_active_servmask_plugins() ) );

		// Set active template
		$config['Template'] = vsc_backup_active_template();

		// Set active stylesheet
		$config['Stylesheet'] = vsc_backup_active_stylesheet();
		...
		// Set server info
		$config['Server'] = array( '.htaccess' => base64_encode( vsc_backup_get_htaccess() ), 'web.config' => base64_encode( vsc_backup_get_webconfig() ) );

		if ( isset( $params['options']['encrypt_backups'] ) ) {
			$config['Encrypted']          = true;
			$config['EncryptedSignature'] = base64_encode( vsc_backup_encrypt_string( VSC_BACKUP_SIGN_TEXT, $params['options']['encrypt_password'] ) );
		}

		// Save package.json file
		$handle = vsc_backup_open( vsc_backup_package_path( $params ), 'w' );
		vsc_backup_write( $handle, json_encode( $config ) );
		vsc_backup_close( $handle );

		// Set progress
		VSC_Backup_Status::info( __( 'Done preparing configuration file.', VSC_BACKUP_PLUGIN_NAME ) );
