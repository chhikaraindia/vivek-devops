...
class VSC_Backup_Import_Validate {

	public static function execute( $params ) {

		// Verify file if size > 2GB and PHP = 32-bit
		if ( ! vsc_backup_is_filesize_supported( vsc_backup_archive_path( $params ) ) ) {
			throw new VSC_Backup_Import_Exception(
				__(
					...
					'<a href="https://help.vivekchhikara.com/knowledgebase/php-32bit/" target="_blank">Technical details</a>',
		...
		if ( isset( $params['total_archive_size'] ) ) {
			...
			$total_archive_size = vsc_backup_archive_bytes( $params );
		}
		...
		// Set progress
		VSC_Backup_Status::info( sprintf( __( 'Unpacking archive...<br />%d%% complete', VSC_BACKUP_PLUGIN_NAME ), $progress ) );
		...
		if ( ! $archive->is_valid() ) {
			throw new VSC_Backup_Import_Exception( __( 'The archive file is corrupted. Follow <a href="https://help.vivekchhikara.com/knowledgebase/corrupted-archive/" target="_blank">this article</a> to resolve the problem.', VSC_BACKUP_PLUGIN_NAME ) );
		}
		...
		if ( $archive->has_not_reached_eof() ) {
			...
			if ( ( $completed = $archive->extract_by_files_array( vsc_backup_storage_path( $params ), array( VSC_BACKUP_PACKAGE_NAME, VSC_BACKUP_MULTISITE_NAME, VSC_BACKUP_DATABASE_NAME ), array(), array(), $file_bytes_written, $file_bytes_offset ) ) ) {
				$file_bytes_offset = 0;
			}
		...
		if ( $archive->has_reached_eof() ) {
			...
			if ( false === is_file( vsc_backup_package_path( $params ) ) ) {
				throw new VSC_Backup_Import_Exception( __( 'Please make sure that your file was exported using <strong>All-in-One WP Migration</strong> plugin. <a href="https://help.vivekchhikara.com/knowledgebase/invalid-backup-file/" target="_blank">Technical details</a>', VSC_BACKUP_PLUGIN_NAME ) );
			}

			// Set progress
			VSC_Backup_Status::info( __( 'Done unpacking archive.', VSC_BACKUP_PLUGIN_NAME ) );
