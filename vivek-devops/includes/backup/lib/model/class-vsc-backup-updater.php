...
class VSC_Backup_Updater {
		...
		foreach ( $updater as $slug => $update ) {
			if ( isset( $extensions[ $slug ], $update['version'], $update['homepage'], $update['download_link'], $update['icons'] ) ) {
				if ( ( $purchase_id = get_option( $extensions[ $slug ]['key'] ) ) ) {
					...
					// Get download URL
					if ( $slug === 'vsc-backup-file-extension' ) {
	...
	public static function check_for_updates() {
		...
		return update_option( VSC_BACKUP_UPDATER, $updater );
	}
	...
	public static function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		foreach ( VSC_Backup_Extensions::get() as $slug => $extension ) {
			...
			if ( $plugin_file === $extension['basename'] ) {
				...
				// Get updater URL
				$updater_url = add_query_arg( array( 'vsc_backup_check_for_updates' => 1, 'vsc_backup_nonce' => wp_create_nonce( 'vsc_backup_check_for_updates' ) ), network_admin_url( 'plugins.php' ) );

				// Check purchase ID
				if ( get_option( $extension['key'] ) ) {
					$plugin_meta[] = VSC_Backup_Template::get_content( 'updater/check', array( 'url' => $updater_url ) );
				} else {
					$plugin_meta[] = VSC_Backup_Template::get_content( 'updater/modal', array( 'url' => $updater_url, 'modal' => $modal_index ) );
				}
				// Check error message
				if ( isset( $updater[ $slug ]['error_message'] ) ) {
					$plugin_meta[] = VSC_Backup_Template::get_content( 'updater/error', array( 'message' => $updater[ $slug ]['error_message'] ) );
				}
			}
		}
