...
class VSC_Backup_Extractor extends VSC_Backup_Archiver {
	...
	public function list_files() {
		...
		if ( @fseek( $this->file_handle, 0, SEEK_SET ) === -1 ) {
			throw new VSC_Backup_Not_Seekable_Exception( sprintf( __( 'Unable to seek to beginning of file. File: %s', VSC_BACKUP_PLUGIN_NAME ), $this->file_name ) );
		}
		...
		while ( $block = @fread( $this->file_handle, 4377 ) ) {
			...
			if ( ( $data = $this->get_data_from_block( $block ) ) ) {
				...
				if ( @fseek( $this->file_handle, $data['size'], SEEK_CUR ) === -1 ) {
					throw new VSC_Backup_Not_Seekable_Exception( sprintf( __( 'Unable to seek to offset of file. File: %s Offset: %d', VSC_BACKUP_PLUGIN_NAME ), $this->file_name, $data['size'] ) );
				}
	 ...
	 *
	 * @throws \VSC_Backup_Not_Directory_Exception
	 * @throws \VSC_Backup_Not_Seekable_Exception
	 *
	 * @return bool
	 */
	public function extract_one_file_to( $location, $exclude_files = array(), $exclude_extensions = array(), $old_paths = array(), $new_paths = array(), &$file_written = 0, &$file_offset = 0 ) {
		if ( false === is_dir( $location ) ) {
			throw new VSC_Backup_Not_Directory_Exception( sprintf( __( 'Location is not a directory: %s', VSC_BACKUP_PLUGIN_NAME ), $location ) );
		}

		// Replace forward slash with current directory separator in location
		$location = vsc_backup_replace_forward_slash_with_directory_separator( $location );
		...
		if ( $file_offset > 0 ) {
			if ( @fseek( $this->file_handle, - $file_offset - 4377, SEEK_CUR ) === -1 ) {
				throw new VSC_Backup_Not_Seekable_Exception( sprintf( __( 'Unable to seek to offset of file. File: %s Offset: %d', VSC_BACKUP_PLUGIN_NAME ), $this->file_name, - $file_offset - 4377 ) );
			}
		}
			...
			if ( $block === $this->eof ) {
				...
				if ( ( $data = $this->get_data_from_block( $block ) ) ) {
					...
					for ( $i = 0; $i < count( $exclude_files ); $i++ ) {
						if ( strpos( $file_name . DIRECTORY_SEPARATOR, vsc_backup_replace_forward_slash_with_directory_separator( $exclude_files[ $i ] ) . DIRECTORY_SEPARATOR ) === 0 ) {
					...
					if ( $should_exclude_file === false ) {
						...
						for ( $i = 0; $i < count( $old_paths ); $i++ ) {
							if ( strpos( $file_path . DIRECTORY_SEPARATOR, vsc_backup_replace_forward_slash_with_directory_separator( $old_paths[ $i ] ) . DIRECTORY_SEPARATOR ) === 0 ) {
								$file_name = substr_replace( $file_name, vsc_backup_replace_forward_slash_with_directory_separator( $new_paths[ $i ] ), 0, strlen( vsc_backup_replace_forward_slash_with_directory_separator( $old_paths[ $i ] ) ) );
								$file_path = substr_replace( $file_path, vsc_backup_replace_forward_slash_with_directory_separator( $new_paths[ $i ] ), 0, strlen( vsc_backup_replace_forward_slash_with_directory_separator( $old_paths[ $i ] ) ) );
								break;
							}
						}
						...
						if ( path_is_absolute( $file_path ) ) {
							$file_path = vsc_backup_escape_windows_directory_separator( $file_path );
						} else {
							$file_path = vsc_backup_escape_windows_directory_separator( $location . DIRECTORY_SEPARATOR . $file_path );
						}

						// Escape Windows directory separator in file name
						if ( path_is_absolute( $file_name ) ) {
							$file_name = vsc_backup_escape_windows_directory_separator( $file_name );
						} else {
							$file_name = vsc_backup_escape_windows_directory_separator( $location . DIRECTORY_SEPARATOR . $file_name );
						}
						...
						if ( @fseek( $this->file_handle, $file_size, SEEK_CUR ) === -1 ) {
							throw new VSC_Backup_Not_Seekable_Exception( sprintf( __( 'Unable to seek to offset of file. File: %s Offset: %d', VSC_BACKUP_PLUGIN_NAME ), $this->file_name, $file_size ) );
						}
					}
				}
			}
			...
			if ( $block === $this->eof ) {
				...
				if ( ( $data = $this->get_data_from_block( $block ) ) ) {
					...
					for ( $i = 0; $i < count( $include_files ); $i++ ) {
						if ( strpos( $file_name . DIRECTORY_SEPARATOR, vsc_backup_replace_forward_slash_with_directory_separator( $include_files[ $i ] ) . DIRECTORY_SEPARATOR ) === 0 ) {
	...
	private function extract_to( $file_name, $file_size, $file_mtime, &$file_written = 0, &$file_offset = 0 ) {
		...
		if ( ( $file_handle = @fopen( $file_name, ( $file_offset === 0 ? 'wb' : 'ab' ) ) ) ) {
			...
			while ( $file_size > 0 ) {
				...
				if ( ! empty( $vsc_backup_params['decryption_password'] ) && basename( $file_name ) !== 'package.json' ) {
					if ( $file_size > 512000 ) {
						$chunk_size += vsc_backup_crypt_iv_length() * 2;
				...
				if ( $chunk_size > 0 ) {
					...
					if ( ( $file_content = @fread( $this->file_handle, $chunk_size ) ) === false ) {
						throw new VSC_Backup_Not_Readable_Exception( sprintf( __( 'Unable to read content from file. File: %s', VSC_BACKUP_PLUGIN_NAME ), $this->file_name ) );
					}
					...
					if ( ! empty( $vsc_backup_params['decryption_password'] ) && basename( $file_name ) !== 'package.json' ) {
						$file_content = vsc_backup_decrypt_string( $file_content, $vsc_backup_params['decryption_password'], $file_name );
					}

					// Write file contents
					if ( ( $file_bytes = @fwrite( $file_handle, $file_content ) ) !== false ) {
						if ( strlen( $file_content ) !== $file_bytes ) {
							throw new VSC_Backup_Quota_Exceeded_Exception( sprintf( __( 'Out of disk space. Unable to write content to file. File: %s', VSC_BACKUP_PLUGIN_NAME ), $file_name ) );
						}
					}
	...
	private function get_data_from_block( $block ) {
		...
		if ( ( $data = unpack( $format, $block ) ) ) {
			...
			$data['filename'] = vsc_backup_replace_forward_slash_with_directory_separator( $data['filename'] );

			// Replace forward slash with current directory separator in file path
			$data['path'] = vsc_backup_replace_forward_slash_with_directory_separator( $data['path'] );
		}
