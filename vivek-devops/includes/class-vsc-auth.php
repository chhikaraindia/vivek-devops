...
class VSC_Auth {
    ...
    public function configure_session() {
        if (session_status() === PHP_SESSION_NONE) {
            ...
            ini_set('session.gc_maxlifetime', 7200); // 2 hours server-side max
            session_start();
        }
    }
    ...
    private function log_honeypot_incident($reason = '') {
        ...
        // Log to WordPress options (temporary storage)
        $honeypot_logs = get_option('vsc_honeypot_logs', []);
        ...
        $honeypot_logs[] = [
            ...
            'reason' => $reason,
            'timestamp' => $timestamp,
            'username_attempt' => $_POST['log'] ?? '',
        ];

        // Keep only last 100 incidents
        if (count($honeypot_logs) > 100) {
            array_shift($honeypot_logs);
        }

        update_option('vsc_honeypot_logs', $honeypot_logs);
    ...
    private function get_user_ip() {
        ...
        return $ip_address ? $ip_address : 'Unknown';
    }
