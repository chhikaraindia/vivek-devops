...
class VSC_Migration_Database_Mysqli extends VSC_Migration_Database {
		...
		if ( ! mysqli_real_query( $this->wpdb->dbh, $input ) ) {
			...
			if ( empty( $this->wpdb->dbh ) || 2006 === $mysqli_errno ) {
				if ( ! $this->wpdb->check_connection( false ) ) {
					throw new VSC_Migration_Database_Exception( __( 'Error reconnecting to the database. <a href="https://help.servmask.com/knowledgebase/mysql-error-reconnecting/" target="_blank">Technical details</a>', 'vivek-devops' ), 503 );
				}
