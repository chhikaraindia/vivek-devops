...
abstract class VSC_Migration_Database {
	...
	public function __construct( $wpdb ) {
		...
		if ( is_resource( $this->wpdb->dbh ) ) {
			if ( get_resource_type( $this->wpdb->dbh ) === 'SQL Server Connection' ) {
				throw new VSC_Migration_Database_Exception(
					__(
	...
	public function export( $file_name, &$query_offset = 0, &$table_index = 0, &$table_offset = 0, &$table_rows = 0 ) {
		// Set file handler
		$file_handler = vsc_migration_open( $file_name, 'cb' );
		...
		if ( fseek( $file_handler, $query_offset ) !== -1 ) {
			...
			// Write headers
			if ( $query_offset === 0 ) {
				vsc_migration_write( $file_handler, $this->get_header() );
			}
			...
			for ( ; $table_index < count( $tables ); ) {
				...
				if ( in_array( $table_name, $views ) ) {
					...
					if ( $table_offset === 0 ) {
						...
						vsc_migration_write( $file_handler, $drop_view );
						...
						vsc_migration_write( $file_handler, $create_view );

						// Write end of statement
						vsc_migration_write( $file_handler, ";\n\n" );
					}
					...
					if ( $table_offset === 0 ) {
						...
						vsc_migration_write( $file_handler, $drop_table );
						...
						vsc_migration_write( $file_handler, $create_table );
					...
					do {
						...
						if ( $primary_keys ) {
							...
							if ( defined( 'VSC_MIGRATION_DISABLE_LATE_ROW_LOOKUPS' ) ) {
								$query = sprintf( 'SELECT %s FROM `%s` AS t1 WHERE %s ORDER BY %s LIMIT %d, %d', $select_columns, $table_name, $table_where, $table_keys, $table_offset, VSC_MIGRATION_MAX_SELECT_RECORDS );
							} else {
								$query = sprintf( 'SELECT %s FROM `%s` AS t1 JOIN (SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d) AS t2 USING (%s)', $select_columns, $table_name, $table_keys, $table_name, $table_where, $table_keys, $table_offset, VSC_MIGRATION_MAX_SELECT_RECORDS, $table_keys );
							}
						} else {
							...
							// Set query with offset and rows count
							$query = sprintf( 'SELECT %s FROM `%s` WHERE %s ORDER BY %s LIMIT %d, %d', $select_columns, $table_name, $table_where, $table_keys, $table_offset, VSC_MIGRATION_MAX_SELECT_RECORDS );
						}
						...
						if ( $num_rows = $this->num_rows( $result ) ) {
							...
							while ( $row = $this->fetch_assoc( $result ) ) {
								...
								if ( $table_offset % VSC_MIGRATION_MAX_TRANSACTION_QUERIES === 0 ) {
									vsc_migration_write( $file_handler, "START TRANSACTION;\n" );
								}
								...
								vsc_migration_write( $file_handler, $table_insert );
								...
								if ( $table_offset % VSC_MIGRATION_MAX_TRANSACTION_QUERIES === 0 ) {
									vsc_migration_write( $file_handler, "COMMIT;\n" );
								}
							}
						} else {
		...
		// Close file handler
		vsc_migration_close( $file_handler );
	...
	public function import( $file_name, &$query_offset = 0 ) {
		...
		// Set file handler
		$file_handler = vsc_migration_open( $file_name, 'rb' );
				...
				if ( preg_match( '/;\s*$/S', $query ) ) {
					...
					if ( strlen( $query ) <= $max_allowed_packet ) {
						...
						if ( $this->should_ignore_query( $query ) === false ) {
							...
							if ( $this->errno() === 1813 ) {
								throw new VSC_Migration_Database_Exception( __( 'Error importing database table. <a href="https://help.servmask.com/knowledgebase/mysql-error-importing-table/" target="_blank">Technical details</a>', 'vivek-devops' ), 503 );
							}
	...
	protected function replace_visual_composer_values_callback( $matches ) {
		...
		if ( VSC_Migration_Database_Utility::base64_validate( $matches[1] ) ) {

			// Decode base64 characters
			$matches[1] = VSC_Migration_Database_Utility::base64_decode( $matches[1] );

			// Replace values
			if ( strlen( $matches[1] ) >= $this->get_old_replace_values_min_length() ) {
				$matches[1] = VSC_Migration_Database_Utility::replace_values( $matches[1], $this->get_old_replace_values(), $this->get_new_replace_values() );
			}

			// Encode base64 characters
			$matches[1] = VSC_Migration_Database_Utility::base64_encode( $matches[1] );
		}
	...
	protected function replace_oxygen_builder_values_callback( $matches ) {
		// Validate base64 data
		if ( VSC_Migration_Database_Utility::base64_validate( $matches[2] ) ) {

			// Decode base64 characters
			$matches[2] = VSC_Migration_Database_Utility::base64_decode( $matches[2] );

			// Replace values
			if ( strlen( $matches[2] ) >= $this->get_old_replace_values_min_length() ) {
				$matches[2] = VSC_Migration_Database_Utility::replace_values( $matches[2], $this->get_old_replace_values(), $this->get_new_replace_values() );
			}

			// Encode base64 characters
			$matches[2] = VSC_Migration_Database_Utility::base64_encode( $matches[2] );
		}
	...
	protected function replace_base64_values_callback( $matches ) {
		...
		if ( VSC_Migration_Database_Utility::base64_validate( $matches[1] ) ) {
			...
			// Replace serialized values
			if ( strlen( $matches[1] ) >= $this->get_old_replace_values_min_length() ) {
				$matches[1] = VSC_Migration_Database_Utility::replace_serialized_values( $matches[1], $this->get_old_replace_values(), $this->get_new_replace_values() );
			}
	...
	protected function replace_table_values_callback( $matches ) {
		...
		$matches[1] = VSC_Migration_Database_Utility::unescape_mysql( $matches[1] );
		...
		$matches[1] = VSC_Migration_Database_Utility::escape_mysql( $matches[1] );
	...
	protected function replace_raw_values( $input ) {
		return VSC_Migration_Database_Utility::replace_values( $input, $this->get_old_replace_raw_values(), $this->get_new_replace_raw_values() );
	}
