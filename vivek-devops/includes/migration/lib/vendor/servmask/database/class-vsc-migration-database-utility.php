...
class VSC_Migration_Database_Utility {
	...
	public static function create_client() {
		...
		if ( $wpdb instanceof WP_SQLite_DB || $wpdb instanceof WP_SQLite_DB\wpsqlitedb ) {
			return new VSC_Migration_Database_Sqlite( $wpdb );
		}

		if ( PHP_MAJOR_VERSION >= 7 ) {
			return new VSC_Migration_Database_Mysqli( $wpdb );
		}

		if ( empty( $wpdb->use_mysqli ) ) {
			return new VSC_Migration_Database_Mysql( $wpdb );
		}
