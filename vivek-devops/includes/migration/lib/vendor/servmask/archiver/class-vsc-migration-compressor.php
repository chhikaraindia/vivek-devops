...
class VSC_Migration_Compressor extends VSC_Migration_Archiver {
	...
	public function add_file( $file_name, $new_file_name = '', &$file_written = 0, &$file_offset = 0 ) {
		...
		$file_name = vsc_migration_replace_forward_slash_with_directory_separator( $file_name );
			...
			if ( ( $block = $this->get_file_block( $file_name, $new_file_name ) ) ) {
				...
				if ( $file_offset === 0 ) {
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new VSC_Migration_Quota_Exceeded_Exception( sprintf( __( 'Out of disk space. Could not write header to file. File: %s', 'vivek-devops' ), $this->file_name ) );
						}
					} else {
						throw new VSC_Migration_Not_Writable_Exception( sprintf( __( 'Could not write header to file. File: %s', 'vivek-devops' ), $this->file_name ) );
					}
				}
				...
				if ( @fseek( $file_handle, $file_offset, SEEK_SET ) !== -1 ) {
					...
					while ( false === @feof( $file_handle ) ) {
						...
						if ( ( $file_content = @fread( $file_handle, 512000 ) ) !== false ) {
							...
							if ( isset( $vsc_migration_params['options']['encrypt_backups'] ) && basename( $file_name ) !== 'package.json' ) {
								$file_content = vsc_migration_encrypt_string( $file_content, $vsc_migration_params['options']['encrypt_password'] );
							}

							if ( ( $file_bytes = @fwrite( $this->file_handle, $file_content ) ) !== false ) {
								if ( strlen( $file_content ) !== $file_bytes ) {
									throw new VSC_Migration_Quota_Exceeded_Exception( sprintf( __( 'Out of disk space. Could not write content to file. File: %s', 'vivek-devops' ), $this->file_name ) );
								}
							} else {
								throw new VSC_Migration_Not_Writable_Exception( sprintf( __( 'Could not write content to file. File: %s', 'vivek-devops' ), $this->file_name ) );
							}
				...
				if ( ( $block = $this->get_file_size_block( $file_offset ) ) ) {
					...
					if ( @fseek( $this->file_handle, - $file_offset - 4096 - 12 - 14, SEEK_CUR ) === -1 ) {
						throw new VSC_Migration_Not_Seekable_Exception( __( 'Your PHP is 32-bit. In order to export your file, please change your PHP version to 64-bit and try again. <a href="https://help.servmask.com/knowledgebase/php-32bit/" target="_blank">Technical details</a>', 'vivek-devops' ) );
					}

					// Write file size to file header
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new VSC_Migration_Quota_Exceeded_Exception( sprintf( __( 'Out of disk space. Could not write size to file. File: %s', 'vivek-devops' ), $this->file_name ) );
						}
					} else {
						throw new VSC_Migration_Not_Writable_Exception( sprintf( __( 'Could not write size to file. File: %s', 'vivek-devops' ), $this->file_name ) );
					}
	...
	private function get_file_block( $file_name, $new_file_name = '' ) {
		...
		if ( ( $stat = @stat( $file_name ) ) !== false ) {
			...
			if ( empty( $new_file_name ) ) {
				$name = vsc_migration_basename( $file_name );
			} else {
				$name = vsc_migration_basename( $new_file_name );
			}
			...
			if ( empty( $new_file_name ) ) {
				$path = vsc_migration_replace_directory_separator_with_forward_slash( vsc_migration_dirname( $file_name ) );
			} else {
				$path = vsc_migration_replace_directory_separator_with_forward_slash( vsc_migration_dirname( $new_file_name ) );
			}
