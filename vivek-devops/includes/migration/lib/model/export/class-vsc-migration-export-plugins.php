...
class VSC_Migration_Export_Plugins {
	...
	public static function execute( $params ) {
		...
		VSC_Migration_Status::info( sprintf( __( 'Archiving %1$d plugin files...<br />%2$d%% complete', 'vivek-devops' ), $total_plugins_files_count, $progress ) );
		...
		$plugins_list = vsc_migration_open( vsc_migration_plugins_list_path( $params ), 'r' );
		...
		if ( fseek( $plugins_list, $plugins_bytes_offset ) !== -1 ) {
			...
			// Loop over files
			while ( list( $file_abspath, $file_relpath, $file_size, $file_mtime ) = vsc_migration_getcsv( $plugins_list ) ) {
