...
class VSC_Migration_Export_Media {
	...
	public static function execute( $params ) {
		...
		VSC_Migration_Status::info( sprintf( __( 'Archiving %1$d media files...<br />%2$d%% complete', 'vivek-devops' ), $total_media_files_count, $progress ) );
		...
		// Get media list file
		$media_list = vsc_migration_open( vsc_migration_media_list_path( $params ), 'r' );
		...
		if ( fseek( $media_list, $media_bytes_offset ) !== -1 ) {
			...
			// Loop over files
			while ( list( $file_abspath, $file_relpath, $file_size, $file_mtime ) = vsc_migration_getcsv( $media_list ) ) {
