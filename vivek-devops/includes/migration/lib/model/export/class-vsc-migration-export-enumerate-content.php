...
class VSC_Migration_Export_Enumerate_Content {

	public static function execute( $params ) {

		$exclude_filters = array_merge( array( vsc_migration_get_uploads_dir(), vsc_migration_get_plugins_dir() ), vsc_migration_get_themes_dirs() );
		...
		// Set progress
		VSC_Migration_Status::info( __( 'Gathering content files...', 'vivek-devops' ) );
		...
		$content_list = vsc_migration_open( vsc_migration_content_list_path( $params ), 'w' );
		...
		if ( isset( $params['options']['no_themes'], $params['options']['no_muplugins'], $params['options']['no_plugins'] ) === false ) {
			...
			$iterator = new VSC_Migration_Recursive_Directory_Iterator( WP_CONTENT_DIR );

			// Exclude content files
			$iterator = new VSC_Migration_Recursive_Exclude_Filter( $iterator, apply_filters( 'vsc_migration_exclude_content_from_export', vsc_migration_content_filters( $exclude_filters ) ) );
			...
			foreach ( $iterator as $item ) {
				if ( $item->isFile() ) {
					if ( vsc_migration_putcsv( $content_list, array( $iterator->getPathname(), $iterator->getSubPathname(), $iterator->getSize(), $iterator->getMTime() ) ) ) {
		...
		// Set progress
		VSC_Migration_Status::info( __( 'Content files gathered.', 'vivek-devops' ) );
		...
		vsc_migration_close( $content_list );
