...
class VSC_Migration_Export_Themes {
	...
	public static function execute( $params ) {
		...
		VSC_Migration_Status::info( sprintf( __( 'Archiving %1$d theme files...<br />%2$d%% complete', 'vivek-devops' ), $total_themes_files_count, $progress ) );
		...
		$themes_list = vsc_migration_open( vsc_migration_themes_list_path( $params ), 'r' );
		...
		if ( fseek( $themes_list, $themes_bytes_offset ) !== -1 ) {
			...
			// Loop over files
			while ( list( $file_abspath, $file_relpath, $file_size, $file_mtime ) = vsc_migration_getcsv( $themes_list ) ) {
