...
class VSC_Migration_Export_Content {
	...
	public static function execute( $params ) {
		...
		VSC_Migration_Status::info( sprintf( __( 'Archiving %1$d content files...<br />%2$d%% complete', 'vivek-devops' ), $total_content_files_count, $progress ) );
		...
		$content_list = vsc_migration_open( vsc_migration_content_list_path( $params ), 'r' );
		...
		if ( fseek( $content_list, $content_bytes_offset ) !== -1 ) {
			...
			// Loop over files
			while ( list( $file_abspath, $file_relpath, $file_size, $file_mtime ) = vsc_migration_getcsv( $content_list ) ) {
