...
class VSC_Migration_Export_Config {

	public static function execute( $params ) {
		global $table_prefix, $wp_version;

		// Set progress
		VSC_Migration_Status::info( __( 'Preparing configuration...', 'vivek-devops' ) );
		...
		// Set plugin version
		$config['Plugin'] = array( 'Version' => VSC_MIGRATION_VERSION );

		// Set WordPress version and content
		$config['WordPress'] = array( 'Version' => $wp_version, 'Absolute' => ABSPATH, 'Content' => WP_CONTENT_DIR, 'Plugins' => vsc_migration_get_plugins_dir(), 'Themes' => vsc_migration_get_themes_dirs(), 'Uploads' => vsc_migration_get_uploads_dir(), 'UploadsURL' => vsc_migration_get_uploads_url() );
		...
		// Set active plugins
		$config['Plugins'] = array_values( array_diff( vsc_migration_active_plugins(), vsc_migration_active_servmask_plugins() ) );

		// Set active template
		$config['Template'] = vsc_migration_active_template();

		// Set active stylesheet
		$config['Stylesheet'] = vsc_migration_active_stylesheet();
		...
		// Set server info
		$config['Server'] = array( '.htaccess' => base64_encode( vsc_migration_get_htaccess() ), 'web.config' => base64_encode( vsc_migration_get_webconfig() ) );

		if ( isset( $params['options']['encrypt_backups'] ) ) {
			$config['Encrypted']          = true;
			$config['EncryptedSignature'] = base64_encode( vsc_migration_encrypt_string( VSC_MIGRATION_SIGN_TEXT, $params['options']['encrypt_password'] ) );
		}

		// Save package.json file
		$handle = vsc_migration_open( vsc_migration_package_path( $params ), 'w' );
		vsc_migration_write( $handle, json_encode( $config ) );
		vsc_migration_close( $handle );

		// Set progress
		VSC_Migration_Status::info( __( 'Configuration prepared.', 'vivek-devops' ) );
