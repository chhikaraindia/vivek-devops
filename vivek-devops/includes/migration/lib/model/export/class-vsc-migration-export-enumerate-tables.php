...
class VSC_Migration_Export_Enumerate_Tables {
	...
	public static function execute( $params ) {
		...
		// Set progress
		VSC_Migration_Status::info( __( 'Gathering database tables...', 'vivek-devops' ) );
		...
		if ( vsc_migration_table_prefix() ) {
			$db_client->add_table_prefix_filter( vsc_migration_table_prefix() );
		...
		$tables_list = vsc_migration_open( vsc_migration_tables_list_path( $params ), 'w' );
		...
		foreach ( $db_client->get_tables() as $table_name ) {
			if ( ! in_array( $table_name, $excluded_db_tables ) && vsc_migration_putcsv( $tables_list, array( $table_name ) ) ) {
				$total_tables_count++;
			}
		}
		...
		if ( isset( $params['options']['include_db_tables'] ) && ! empty( $params['included_db_tables'] ) ) {
			foreach ( explode( ',', $params['included_db_tables'] )  as $table_name ) {
				if ( vsc_migration_putcsv( $tables_list, array( $table_name ) ) ) {
					$total_tables_count++;
				}
			}
		}
		...
		// Set progress
		VSC_Migration_Status::info( __( 'Database tables gathered.', 'vivek-devops' ) );
