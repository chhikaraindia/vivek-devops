...
class VSC_Migration_Import_Blogs {

	public static function execute( $params ) {

		// Set progress
		VSC_Migration_Status::info( __( 'Preparing blogs...', 'vivek-devops' ) );

		$blogs = array();

		// Check multisite.json file
		if ( true === is_file( vsc_migration_multisite_path( $params ) ) ) {
					...
					if ( count( $sites ) === 1 && ( $subsite = current( $sites ) ) ) {
						...
						$blogs[] = array(
							...
							'New' => array(
								...
								'WordPress'       => array(
									'UploadsURL' => vsc_migration_get_uploads_url(),
								),
							),
						);
					} else {
						throw new VSC_Migration_Import_Exception( esc_html__( 'The archive must contain only a single WordPress site. The process cannot continue. Please revisit your export settings.', 'vivek-devops' ) );
					}
				} else {
					throw new VSC_Migration_Import_Exception( esc_html__( 'The archive must contain at least one WordPress site. The process cannot continue. Please check your export settings.', 'vivek-devops' ) );
				}
			} else {
				throw new VSC_Migration_Import_Exception( esc_html__( 'Could not import a WordPress Network into a single WordPress site. The process cannot continue. Please check your import settings.', 'vivek-devops' ) );
			}
		}

		// Write blogs.json file
		$handle = vsc_migration_open( vsc_migration_blogs_path( $params ), 'w' );
		vsc_migration_write( $handle, json_encode( $blogs ) );
		vsc_migration_close( $handle );

		// Set progress
		VSC_Migration_Status::info( __( 'Blogs prepared.', 'vivek-devops' ) );
