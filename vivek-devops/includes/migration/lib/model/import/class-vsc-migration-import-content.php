...
class VSC_Migration_Import_Content {
	...
	public static function execute( $params ) {
		...
		VSC_Migration_Status::info( sprintf( __( 'Restoring %1$d files...<br />%2$d%% complete', 'vivek-devops' ), $total_files_count, $progress ) );
		...
		$new_paths = array( vsc_migration_get_plugins_dir(), get_theme_root() );
		...
		foreach ( $blogs as $blog ) {
			if ( vsc_migration_is_mainsite( $blog['Old']['BlogID'] ) === false ) {
				if ( defined( 'UPLOADBLOGSDIR' ) ) {
					// Old files dir style
					$old_paths[] = vsc_migration_blog_files_relpath( $blog['Old']['BlogID'] );
					$new_paths[] = vsc_migration_blog_files_abspath( $blog['New']['BlogID'] );

					// Old blogs.dir style
					$old_paths[] = vsc_migration_blog_blogsdir_relpath( $blog['Old']['BlogID'] );
					$new_paths[] = vsc_migration_blog_blogsdir_abspath( $blog['New']['BlogID'] );

					// New sites dir style
					$old_paths[] = vsc_migration_blog_sites_relpath( $blog['Old']['BlogID'] );
					$new_paths[] = vsc_migration_blog_files_abspath( $blog['New']['BlogID'] );
				} else {
					// Old files dir style
					$old_paths[] = vsc_migration_blog_files_relpath( $blog['Old']['BlogID'] );
					$new_paths[] = vsc_migration_blog_sites_abspath( $blog['New']['BlogID'] );
		...
		$old_paths[] = vsc_migration_blog_sites_relpath();
		$new_paths[] = vsc_migration_blog_sites_abspath();
		...
		while ( $archive->has_not_reached_eof() ) {
			...
			$exclude_files = array_merge(
				...
				array(
					...
					VSC_MIGRATION_MUPLUGINS_NAME,
				)
			);

			// Exclude theme files
			$exclude_files = array_merge( $exclude_files, array( VSC_MIGRATION_THEMES_FUNCTIONS_NAME ) );

			// Exclude Elementor files
			$exclude_files = array_merge( $exclude_files, array( VSC_MIGRATION_ELEMENTOR_CSS_NAME ) );

			// Exclude CiviCRM files
			$exclude_files = array_merge( $exclude_files, array( VSC_MIGRATION_CIVICRM_UPLOADS_NAME ) );

			// Exclude content extensions
			$exclude_extensions = array( VSC_MIGRATION_LESS_CACHE_EXTENSION, VSC_MIGRATION_SQLITE_DATABASE_EXTENSION );
