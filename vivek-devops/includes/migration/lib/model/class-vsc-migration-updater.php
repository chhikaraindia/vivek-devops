...
class VSC_Migration_Updater {
	...
	public static function check_for_updates() {
		...
		return update_option( VSC_MIGRATION_UPDATER, $updater );
	}
	...
	public static function plugin_row_meta( $plugin_meta, $plugin_file ) {
		...
		foreach ( VSC_Migration_Extensions::get() as $slug => $extension ) {
			if ( $plugin_file === $extension['basename'] ) {
				...
				// Get updater URL
				$updater_url = add_query_arg( array( 'vsc_migration_check_for_updates' => 1, 'vsc_migration_nonce' => wp_create_nonce( 'vsc_migration_check_for_updates' ) ), network_admin_url( 'plugins.php' ) );

				// Check for updates
				$plugin_meta[] = VSC_Migration_Template::get_content( 'updater/check', array( 'url' => $updater_url ) );

				// Check error message
				if ( isset( $updater[ $slug ]['error_message'] ) ) {
					$plugin_meta[] = VSC_Migration_Template::get_content( 'updater/error', array( 'message' => $updater[ $slug ]['error_message'] ) );
				}
			}
		}
