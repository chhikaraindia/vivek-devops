...
class VSC_Migration_Compatibility {
	...
	public static function get( $params ) {
		...
		$messages = array();
		foreach ( $extensions as $extension_name => $extension_data ) {
			if ( ! VSC_Migration_Compatibility::check( $extension_data ) ) {
				if ( defined( 'WP_CLI' ) ) {
					/* translators: Extension name. */
					$messages[] = sprintf( __( '%s is out of date. Please update this extension before using it.', 'vivek-devops' ), $extension_data['title'] );
				} else {
					/* translators: 1: Extension name, 2: Plugins update page. */
					$messages[] = sprintf( __( '<strong>%1$s</strong> is out of date. You must <a href="%2$s">update this extension</a> before using it.<br />', 'vivek-devops' ), $extension_data['title'], network_admin_url( 'plugins.php' ) );
				}
			}
		}
